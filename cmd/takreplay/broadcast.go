...
func NewBroadcastDumper(speedUp int) *BroadcastDumper {
...
func (d *BroadcastDumper) Process(msg *cot.CotMessage) error {
	...
	evt.StaleTime = cot.TimeToMillis(now.Add(stale / time.Duration(d.speedUp)))
	...
	if t > 0 {
		time.Sleep(t / time.Duration(d.speedUp))
	}
